<!-- Synced from Confluence page 6428983350: confluence-submit 2026-07-08 14:52 -->

---

## name: confluence-submit description: > Trigger this skill whenever the user says "/submit" — with or without anything else in the message. This skill audits the skill being discussed with a built-in five-factor scoring rubric, asks the user whether to file it as Enterprise or Team, runs a second placement check against the chosen filing location, checks the IAS space for duplicate or overlapping skills, then publishes it as a new child page in the Internal AI Skills Confluence space (IAS) — directly under the Enterprise page, or under the chosen Team subpage (e.g. Biz Ops, Sales, Marketing) — along with a companion audit-summary child page that gives reviewers a fast, scannable view of the skill's strengths and gaps. Every published page title includes the submission date. Use this skill immediately when "/submit" appears anywhere in the user's message. Confirmation is required for the Enterprise/Team choice, the subpage choice, and any duplicate or overlap found, but not for the act of publishing itself once those choices are made.

# /submit — Publish Skill to Confluence

When the user says `/submit`, audit the skill currently being discussed, ask where it should be  
filed, double-check that placement, then publish it to Confluence under the right page with a  
dated title.
...
## Step 3: First Audit — Quality Verdict
...
Note: this classification is informative only — per Step 4, the user always makes the actual Enterprise/Team filing decision regardless of what's concluded here.

### 3c. Apply the Right Framework
...
## Step 4: Ask Enterprise or Team

Regardless of what the audit in Step 3 concluded (even if it classified the skill as Individual, or strongly reads as Enterprise- or Team-scoped), always explicitly ask the user whether this skill should be filed as Enterprise or Team. The auditor's classification is informative context to mention, but the filing decision is the user's call, not automatic.

---

## Step 5: Resolve the Filing Location

**If Enterprise:**

The skill is filed directly under the Enterprise page. Look up the Enterprise page dynamically (see "Locating filing pages" below) — do not assume a fixed page ID. Skip to Step 6.
...
## Step 6: Second Audit — Placement Check
...
## Step 7: Check for Duplicate or Overlapping Skills

Before publishing, check whether the IAS space already has a skill that does substantially the same job as this one. The goal isn't to block similar-sounding skills — it's to prevent two skills from doing the same task with only cosmetic differences, which fragments the library and confuses whoever's picking a skill to use.

**7a. Find candidates.** Call `getPagesInConfluenceSpace` (cloudId `20d966ed-3d3f-4c0c-b14b-c1dbb9bb6aa7`, spaceId `6386384916`) to list titles across the whole space — not just the destination page, since an overlapping skill could live anywhere (Enterprise or any Team subpage). Also run `searchConfluenceUsingCql` with a couple of keyword variants pulled from the skill's core task (not just its literal name — two skills solving the same problem often have different names). Skip structural pages (space homepage, "Enterprise Skills," "Team Skills," topic subpages like "Sales") — you're looking for actual skill pages.

**7b. Compare on substance, not surface.** For any candidate whose title or summary looks plausibly related, fetch it with `getConfluencePage` and compare against the skill being submitted on:
...
* **No meaningful overlap:** State briefly that no overlapping skill was found, and continue to Step 8.
* **Clear duplicate or near-duplicate** (same core task, same triggers, only superficial differences in wording/format): Do not publish yet. Tell the user plainly which existing skill(s) overlap and why, and recommend a path — merge the two, narrow one skill's scope so each has a distinct, non-overlapping use case, or revise the trigger conditions to stop them from colliding. Ask the user how they want to proceed (revise and resubmit, publish anyway, or cancel) before taking any further action.
* **Partial or ambiguous overlap:** Name the related skill(s), describe the overlap concisely, and ask the user to confirm whether to proceed as-is or revise first. Don't guess on their behalf.

Every skill in the space should have its own definitive use case and be able to stand on its own — this check exists to protect that, not to add friction for its own sake. Keep the report to a few sentences; this isn't a full audit rewrite.
...
* **Parent page ID:** the Enterprise page (Enterprise filing) or the chosen Team subpage (Team filing), resolved in Step 5
...
* **Body:** the full raw `SKILL.md` content, exactly as written — no summarizing, no paraphrasing, no abbreviating

Call `createConfluencePage` with these parameters.

---

## Step 9: Publish an Audit Summary as a Child Page
...
Use the scores and feedback already produced in Step 3 — do not re-run or re-score the audit. Condense it down to:

* **Where it's strong:** the 1–2 highest-scoring factors, one sentence each, drawn from the Step 3 feedback.
* **Where it falls short:** the 1–2 lowest-scoring factors (or any that failed their threshold), one sentence each, drawn from the Step 3 feedback. If every factor passed comfortably, note the weakest-but-passing factor instead so there's always something concrete for a reviewer to check.
* **Recommendations:** carry over the Top Recommendations from Step 3, verbatim or lightly condensed.
...
## Step 10: Publish Supplementary Content as Child Pages
...
* **Parent page ID:** the skill page created in Step 8 (not the Enterprise/Team page)
...
After all are published, briefly note in the Step 12 confirmation that supplementary content was included, with links.

---

## Step 11: Handle Duplicate Titles

Since titles already include the date, collisions should be rare. If the API still returns a `409 Conflict` or duplicate title error (e.g. two submissions of the same skill on the same day), append a short time suffix (e.g. `14:32`) to the title and retry once. This applies to the main skill page, the audit summary from Step 9, and any supplementary child pages from Step 10.

---

## Step 12: Confirm with the URL
...
If supplementary content was published in Step 10, add one line per file:
