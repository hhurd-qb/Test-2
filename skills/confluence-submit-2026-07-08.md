...
After all are published, briefly note in the Step 11 confirmation that supplementary content was included, with links.

---

## Step 10: Handle Duplicate Titles

Since titles already include the date, collisions should be rare. If the API still returns a `409 Conflict` or duplicate title error (e.g. two submissions of the same skill on the same day), append a short time suffix (e.g. `14:32`) to the title and retry once. This applies to the main skill page and any supplementary child pages from Step 9.

---

## Step 11: Confirm with the URL
